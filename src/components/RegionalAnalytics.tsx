...
export const RegionalAnalytics: React.FC = () => {
  ...
  const regionalComparisonData = useMemo(() => {
    ...
    const result = orderedRegions.map(regionName => {
      ...
      const regionData = {
        region: regionName,
        tcc_p25: calculateAverage(regionRows.map(r => getTccP25(r))),
        tcc_p50: calculateAverage(regionRows.map(r => getTccP50(r))),
        tcc_p75: calculateAverage(regionRows.map(r => getTccP75(r))),
        tcc_p90: calculateAverage(regionRows.map(r => getTccP90(r))),
        cf_p25: calculateAverage(regionRows.map(r => getCfP25(r))),
        cf_p50: calculateAverage(regionRows.map(r => getCfP50(r))),
        cf_p75: calculateAverage(regionRows.map(r => getCfP75(r))),
        cf_p90: calculateAverage(regionRows.map(r => getCfP90(r))),
        wrvus_p25: calculateAverage(regionRows.map(r => getWrvuP25(r))),
        wrvus_p50: calculateAverage(regionRows.map(r => getWrvuP50(r))),
        wrvus_p75: calculateAverage(regionRows.map(r => getWrvuP75(r))),
        wrvus_p90: calculateAverage(regionRows.map(r => getWrvuP90(r))),
      };
      ...
      if (regionName.toLowerCase() === 'national') {
        console.log(`🔍 National calculation details:`, {
          ...
          allTCCP50Values: regionRows.map(r => getTccP50(r)).slice(0, 10),
