...
export class AnalyticsDataService {
      ...
      if (shouldProcess) {
        ...
        if (isCallPayVariable || (isCallPaySurvey && (originalVariable.toLowerCase().includes('on') && originalVariable.toLowerCase().includes('call')))) {
          console.log('🔍 Call Pay variable extracted in normalizeRowDynamic:', {
            surveyId: survey.id,
            surveyName: survey.name,
            surveySource: (survey as any).source || survey.type,
            dataCategory: (survey as any).dataCategory,
            providerType: survey.providerType,
            originalVariable,
            mappedVariable: variable,
            ...
            selectedVariables: selectedVariables.length > 0 ? selectedVariables : 'ALL'
          });
          
          // Warning if variable should be processed but has no data
          if (shouldProcess && p50 === 0 && p25 === 0 && p75 === 0 && p90 === 0) {
            console.warn('⚠️ Call Pay variable extracted but has ZERO values:', {
              surveyName: survey.name,
              originalVariable,
              normalizedVarName,
              rowData: {
                p25: actualRowData.p25,
                p50: actualRowData.p50,
                p75: actualRowData.p75,
                p90: actualRowData.p90
              }
            });
          }
        }
  ...
  private async aggregateByVariables(
    ...
    normalizedRows.forEach(row => {
      ...
      // Debug logging for Work RVUs to diagnose aggregation issues
      if (row.variables && (row.variables['work_rvus'] || row.variables['work_rvu'] || row.variables['wrvu'])) {
        const wrvuVar = row.variables['work_rvus'] || row.variables['work_rvu'] || row.variables['wrvu'];
        console.log('🔍 Work RVU Row Grouping:', {
          key,
          surveySource: row.surveySource,
          specialty: row.standardizedSpecialty,
          providerType: row.providerType,
          region: row.region,
          wrvuP50: wrvuVar?.p50,
          wrvuP25: wrvuVar?.p25,
          wrvuP75: wrvuVar?.p75,
          wrvuP90: wrvuVar?.p90,
          n_incumbents: wrvuVar?.n_incumbents,
          variableKeys: Object.keys(row.variables)
        });
      }
    });
    ...
    for (const [, rows] of groupedData) {
      ...
      // Create aggregated metrics for each variable - properly aggregate when multiple rows exist
      variableMap.forEach((metricsList, varName) => {
        if (metricsList.length === 0) return;
        
        // If only one metric, use it directly
        if (metricsList.length === 1) {
          ...
          // Multiple metrics - calculate weighted averages using n_incumbents as weights
          let totalOrgs = 0;
          ...
          metricsList.forEach(metrics => {
            ...
            // Only include percentiles that have actual data (> 0)
            if (metrics.p25 > 0) {
          ...
          const calculateWeightedAverage = (values: Array<{ value: number; weight: number }>): number => {
          ...
          aggregatedRecord.variables[varName] = {
            ...
            p25: calculateWeightedAverage(p25Values),
            p50: calculateWeightedAverage(p50Values),
            p75: calculateWeightedAverage(p75Values),
            p90: calculateWeightedAverage(p90Values)
          };
          
          // Log when multiple metrics are aggregated
          console.log('🔍 Aggregated multiple metrics for variable:', {
            variableName: varName,
            surveySource: aggregatedRecord.surveySource,
            specialty: aggregatedRecord.standardizedName,
            region: aggregatedRecord.geographicRegion,
            metricsCount: metricsList.length,
            totalIncumbents,
            aggregatedMetrics: aggregatedRecord.variables[varName]
          });
        }
        
        // Log on-call variables during aggregation
        if (varName === 'on_call_compensation' || varName.includes('on_call') || varName.includes('oncall')) {
          console.log('✅ On-Call Variable Aggregated:', {
            surveySource: aggregatedRecord.surveySource,
            dataCategory: aggregatedRecord.dataCategory,
            specialty: aggregatedRecord.standardizedName,
            region: aggregatedRecord.geographicRegion,
            variableName: varName,
            metrics: aggregatedRecord.variables[varName],
