...
export interface UseBenchmarkingQueryReturn {
  data: AnalyticsData[]; // Filtered data for display
  allData: AnalyticsData[]; // All data for filter options
  loading: boolean;
...
async function fetchBenchmarkingData(
  ...
  try {
    ...
    console.log('🔍 fetchBenchmarkingData: Using dynamic variable method with selectedVariables:', selectedVariables);
    
    // CRITICAL FIX: Always use getAnalyticsDataByVariables for dynamic variable support
    // This ensures on-call compensation and other dynamic variables are properly extracted
    // Passing empty array processes ALL variables, which is what we want for benchmarking
    const [allData, specialtyMappings, columnMappings, regionMappings] = await Promise.all([
      analyticsDataService.getAnalyticsDataByVariables({
        ...
        providerType: '',
        year: '',
        dataCategory: '' // Empty means all categories including CALL_PAY
      }, selectedVariables.length > 0 ? selectedVariables : []), // Empty array = process all variables
      dataService.getAllSpecialtyMappings(),
      dataService.getAllColumnMappings(),
      dataService.getRegionMappings()
    ]);
    
    // Log summary of on-call variables found
    const onCallRows = allData.filter(row => {
      if ((row as any).variables) {
        return Object.keys((row as any).variables).some(k => 
    ...
    if (onCallRows.length > 0) {
      console.log(`✅ fetchBenchmarkingData: Found ${onCallRows.length} rows with on-call variables`);
    } else {
      console.warn('⚠️ fetchBenchmarkingData: NO rows with on-call variables found in aggregated data');
    }
...
export const useBenchmarkingQuery = (
  ...
  // CRITICAL FIX: Include selectedVariables in query key to ensure cache invalidation when variables change
  const queryKeyWithVariables = [...queryKey, selectedVariables.sort().join(',')];
  
  // Query options with caching policy
  const query = useQuery<BenchmarkingQueryData>({
    queryKey: queryKeyWithVariables,
    queryFn: createQueryFn((signal) => fetchBenchmarkingData(initialFilters, selectedVariables, signal)),
    enabled: true,
