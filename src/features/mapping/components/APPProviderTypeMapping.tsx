...
export const APPProviderTypeMapping: React.FC<APPProviderTypeMappingProps> = ({
  ...
  const loadMappings = async () => {
    try {
      ...
      const stored = await dataService.getUserPreference('app_provider_type_mappings');
  ...
  const saveMappings = async (mappingsToSave: APPProviderTypeMapping[]) => {
    try {
      const dataService = getDataService();
      await dataService.saveUserPreference('app_provider_type_mappings', mappingsToSave);
    } catch (err) {
        ...
        <div className="border-b border-gray-200 mb-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <nav className="-mb-px flex space-x-8">
              {[
                { key: 'unmapped', label: `Unmapped Types (${unmappedTypes.length})` },
                { key: 'mapped', label: `Mapped Types (${mappings.length})` },
          ...
          <div className="flex items-center space-x-3 mb-4">
            ...
            {activeTab === 'mapped' && (
              <button
                onClick={async () => {
                  if (window.confirm('Are you sure you want to clear all provider type mappings?')) {
                    setMappings([]);
                    await saveMappings([]);
                  }
                }}
                  ...
                  <div className="text-center max-w-xl w-full border border-dashed border-gray-300 rounded-xl p-10 bg-gray-50">
                    ...
                    <p className="text-gray-600 mb-4">
                      {learnedSearchTerm 
                        ...
                        : 'Learned mappings will appear here as you make corrections to provider type mappings.'
                      }
                    </p>
                  </div>
                </div>
