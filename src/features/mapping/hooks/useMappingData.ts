...
export const useMappingData = (): UseMappingDataReturn => {
  ...
  // Default to true (showing all) to match provider-type-mapping behavior
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Match the exact logic from useProviderTypeMappingData for consistency
      ...
      console.log('🔍 useMappingData: Filtering logic:', {
        showAllProviderTypes,
        selectedProviderType,
        dataProviderType: dataProviderType || 'ALL'
      });
  ...
  // CRITICAL FIX: Watch for changes in showAllProviderTypes directly, not just loadData
  useEffect(() => {
    const dataProviderType = (showAllProviderTypes || selectedProviderType === 'BOTH') 
      ? undefined 
      : selectedProviderType;
    console.log('🔍 useMappingData: Reloading data due to state change:', {
      ...
      showAllProviderTypes,
      dataProviderType: dataProviderType || 'ALL',
      willFilterCallPay: dataProviderType === 'PHYSICIAN'
    });
    loadData();
