/**
 * Optimized Region Mapping Data Hook
...
interface UseOptimizedRegionMappingDataReturn {
  // State
  mappings: any[];
  unmappedRegions: any[];
...
/**
 * Optimized hook for managing region mapping data
 */
export const useOptimizedRegionMappingData = (): UseOptimizedRegionMappingDataReturn => {
  // Provider context
  const { selectedProviderType } = useProviderContext();
  
  // Core state
  const [mappings, setMappings] = useState<any[]>([]);
  const [unmappedRegions, setUnmappedRegions] = useState<any[]>([]);
  ...
  }, [loading, unmappedRegions.length, learnedMappings]);

  // Computed values with memoization
  const filteredUnmapped = useMemo(() => {
    if (!searchTerm) return unmappedRegions;
    return unmappedRegions.filter(item => 
  ...
  const refreshData = useCallback(async () => {
    console.log('🔄 Refreshing region mapping data...');
