/**
 * Optimized Provider Type Mapping Data Hook
...
interface UseOptimizedProviderTypeMappingDataReturn {
  // State
  mappings: any[];
  unmappedProviderTypes: any[];
...
/**
 * Optimized hook for managing provider type mapping data
 */
export const useOptimizedProviderTypeMappingData = (): UseOptimizedProviderTypeMappingDataReturn => {
  // Provider context
  const { selectedProviderType } = useProviderContext();
  
  // Core state
  const [mappings, setMappings] = useState<any[]>([]);
  const [unmappedProviderTypes, setUnmappedProviderTypes] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (!loading) {
      if (unmappedProviderTypes.length > 0) {
  ...
  }, [loading, unmappedProviderTypes.length, learnedMappings]);

  // Computed values with memoization
  const filteredUnmapped = useMemo(() => {
    if (!searchTerm) return unmappedProviderTypes;
    return unmappedProviderTypes.filter(item => 
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      console.log('🚀 Loading optimized provider type mapping data...');
      ...
      console.error('Error loading optimized provider type mapping data:', err);
      setError('Failed to load provider type mapping data');
    } finally {
      setLoading(false);
    }
  ...
  const refreshData = useCallback(async () => {
    console.log('🔄 Refreshing provider type mapping data...');
