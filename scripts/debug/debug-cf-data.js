/**
 * Debug script to check CF data in Regional Analytics
 * Run this in the browser console
 */

async function debugCFData() {
  try {
    console.log('🔍 Debugging CF data in Regional Analytics...');
    ...
    for (const survey of surveys) {
      ...
      const surveyData = await dataService.getSurveyData(survey.id, {}, { limit: 50 });
      ...
      if (surveyData.rows.length > 0) {
        ...
        if (variableRows.length > 0) {
          ...
          variableRows.slice(0, 10).forEach((row, index) => {
        ...
        if (directCFRows.length > 0) {
          ...
          directCFRows.slice(0, 5).forEach((row, index) => {
    ...
    console.log('\n✅ CF data debugging completed');
    
  } catch (error) {
    console.error('❌ Error debugging CF data:', error);
  }
}

// Export for console use
if (typeof window !== 'undefined') {
  window.debugCFData = debugCFData;
  console.log('🔍 Run debugCFData() to debug CF data');
} else {
